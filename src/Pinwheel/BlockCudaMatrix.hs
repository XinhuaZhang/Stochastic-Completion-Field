...
matMul lock doesTranspose deviceID rows matA' matB = do
  ...
  output `seq`
    when (L.length matA' == 1) (freeCuMat . L.head $ matA) `seq`
    BLAS.destroy handle `seq` CUDA.destroy ctx `seq` return output
...
blockMatrixMultiply doesTranspose deviceIDs matAs matBss = do
  ...
  output <-
    fmap concatCuMat .
    MP.mapM (\(deviceID, matBs) -> matMul lock True deviceID rows matAs matBs) .
    L.zip deviceIDs $
    matBss
  return $!
    if doesTranspose
