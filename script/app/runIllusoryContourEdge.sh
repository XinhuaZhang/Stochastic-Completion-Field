...
stack build Stochastic-Completion-Field:IllusoryContourEdge-exe

time stack exec -- IllusoryContourEdge-exe ${GPUID} ${Size} ${Delta} ${Threshold} ${SizeRecon} ${DeltaRecon} ${Orientations} ${Scales} ${ThetaSigma} ${ScaleSigma} ${Tau} ${NumTrails} ${DeltaT} ${PoissonWeight} ${NumR2Freq} ${PeriodR2} ${DeltaFreq} ${PhiFreqs} ${RhoFreqs} ${ThetaFreqs} ${ScaleFreqs} ${InitScale} ${HistPath} ${STDR2} ${STDTheta} ${STDR} ${NumBatchR2} ${NumBatchR2Freqs} ${NumBatchOri} ${BatchSize} ${S} ${WriteFlag} ${Iterations} ${Radius} $1 ${ScaleFactor} ${NumNoisePoint} ${NoisePointRange} ${Threads} +RTS -N${Threads} -RTS
